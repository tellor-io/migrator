...
contract Main {
    ...
    address public uniswaPool = 0x70258Aa9830C2C84d855Df1D61E12C256F6448b4;
    ...
    function migratePool(address poolAddr) external {
        // require(poolAddr == uniswaPool, "must be the uniswap pool");
        uint256 balance = pools[uniswaPool].trbBalanceOf(poolAddr, msg.sender);
        require(balance > 0, "no balance to migrate");
        require(pools[uniswaPool].burn(poolAddr, msg.sender), "burn failed");
